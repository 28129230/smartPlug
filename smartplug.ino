...
void setup()
{
	for(int i=2;i<=9;i++)//初始化数字引脚2-9为高电平，控制8个继电器状态为关闭
	{
		pinMode(i,OUTPUT);
		digitalWrite(i,HIGH);
	}
...
void TVPlug(int state)//电视机插口开关函数
{
	...
	{
		digitalWrite(2,LOW);//使用数字引脚2为低电平控制继电器引脚1，开关通电
		TVPlugState=1;//将电视插口状态置为1，表示通电状态
	}
	else//状态置为0表示插口断电
	{
		digitalWrite(2,HIGH);//使用数字引脚2为高电平控制继电器引脚1，开关关闭
...
void StereoAMPPlug(int state)//立体声功放（包括前级、后级）插口开关函数
{
	if (state == 1)// 状态置为1表示插口通电
	{
		digitalWrite(3,LOW);//使用数字引脚3为低电平控制继电器引脚2，开关通电
		StereoAMPPlugState=1;//将插口状态置为1，表示通电状态
	}
	else//状态置为0表示插口断电
	{
		digitalWrite(3,HIGH);//使用数字引脚3为高电平控制继电器引脚2，开关关闭
...
void AvAMPPlug(int state)//AV功放（包括前级、后级）插口开关函数
{
	if (state == 1)// 状态置为1表示插口通电
	{
		digitalWrite(4,LOW);//使用数字引脚3为低电平控制继电器引脚3，开关通电
		AvAMPPlugState=1;//将插口状态置为1，表示通电状态
	}
	else//状态置为0表示插口断电
	{
		digitalWrite(4,HIGH);//使用数字引脚3为高电平控制继电器引脚3，开关关闭
...
void CTSTPlug(int state)//有线电视机顶盒（Cable TV set top）插口开关函数
{
	if (state == 1)// 状态置为1表示插口通电
	{
		digitalWrite(5,LOW);//使用数字引脚5为低电平控制继电器引脚4，开关通电
		CTSTPlugState=1;//将有线电视机顶盒插口状态置为1，表示通电状态
	}
	else//状态置为0表示插口断电
	{
		digitalWrite(5,HIGH);//使用数字引脚5为高电平控制继电器引脚4，开关关闭
...
void NetPlug(int state)//宽带路由器与交换机、网络存储等网络设备插口开关函数
{
	if (state == 1)// 状态置为1表示插口通电
	{
		digitalWrite(6,LOW);//使用数字引脚7为低电平控制继电器引脚5，开关通电
		NetPlugState=1;//将插口状态置为1，表示通电状态
	}
	else//状态置为0表示插口断电
	{
		digitalWrite(6,HIGH);//使用数字引脚7为高电平控制继电器引脚5，开关关闭
...
void NetSTPlug(int state)//网络机顶盒插口开关函数
{
	if (state == 1)// 状态置为1表示插口通电
	{
		digitalWrite(7,LOW);//使用数字引脚8为低电平控制继电器引脚6，开关通电
		NetSTPlugState=1;//将插口状态置为1，表示通电状态
	}
	else//状态置为0表示插口断电
	{
		digitalWrite(7,HIGH);//使用数字引脚8为高电平控制继电器引脚6，开关关闭
...
void DiskPlayPlug(int state)//碟机（包括CD、DVD、唱盘等）插口开关函数
{
	if (state == 1)// 状态置为1表示插口通电
	{
		digitalWrite(8,LOW);//使用数字引脚3为低电平控制继电器引脚7，开关通电
		DiskPlayPlugState=1;//将插口状态置为1，表示通电状态
	}
	else//状态置为0表示插口断电
	{
		digitalWrite(8,HIGH);//使用数字引脚3为高电平控制继电器引脚7，开关关闭
...
void otherPlug(int state)//其他插口开关函数
{
	if(state==1)//状态置为1表示插口通电
	{
		digitalWrite(9,LOW);//使用数字引脚9为低电平控制继电器引脚8，开关通电
		otherPlugState=1;//将插口状态置为1，表示通电状态
	}
	else
	{
		digitalWrite(9,HIGH);//使用数字引脚9为高电平控制继电器引脚8，开关关闭
